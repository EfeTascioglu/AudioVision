...
namespace PassthroughCameraSamples.MultiObjectDetection
{
    ...
    public class SentisInferenceRunManager : MonoBehaviour
    {
        ...
        private void Awake()
        {
            Debug.Log("[FaceDetection] Awake() started");
            if (m_sentisModel == null)
            {
                Debug.LogError("[FaceDetection] m_sentisModel is not assigned! Assign yolov9face-sentis in the Inspector.");
                return;
            }
            try
            {
                ...
                Debug.Log($"[FaceDetection] Model loaded, input size: {m_inputSize.x}x{m_inputSize.y}");
            }
            catch (System.Exception e)
            {
                Debug.LogError($"[FaceDetection] Model load failed: {e.Message}\n{e.StackTrace}");
            }
        }
        ...
        private IEnumerator RunInference()
        {
            ...
            if (boxes.shape[0] == 0)
            {
                Debug.Log("[FaceDetection] Model returned 0 boxes - check model/output format");
                yield break;
            }
            ...
            // Debug: if model returned boxes but NMS filtered all out, threshold may be too high
            if (boxes.shape[0] > 0 && m_detections.Count == 0)
            {
                float maxScore = 0f;
                var scoresArr = scores.AsReadOnlyNativeArray();
                for (int i = 0; i < scoresArr.Length; i++)
                    if (scoresArr[i] > maxScore) maxScore = scoresArr[i];
                Debug.Log($"[FaceDetection] {boxes.shape[0]} raw boxes, 0 passed NMS. Max score={maxScore:F3}, threshold={m_scoreThreshold}. Try lowering m_scoreThreshold.");
            }
            ...
            if (!m_cameraAccess.IsPlaying || m_detectionManager.m_spatialAnchor == null || !m_detectionManager.m_spatialAnchor.IsTracked)
            {
                if (m_detections.Count > 0)
                    Debug.Log($"[FaceDetection] {m_detections.Count} faces detected but spatial anchor not ready (null={m_detectionManager.m_spatialAnchor == null}, tracked={m_detectionManager.m_spatialAnchor?.IsTracked})");
                yield break;
            }
