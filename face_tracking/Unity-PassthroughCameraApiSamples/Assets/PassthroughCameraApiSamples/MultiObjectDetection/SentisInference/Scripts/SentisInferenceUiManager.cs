    ...
    public class SentisInferenceUiManager : MonoBehaviour
    {
        ...
        public void DrawUIBoxes(List<(int classId, Vector4 boundingBox)> detections, Vector2 inputSize, Pose cameraPose)
        {
            ...
            for (var i = 0; i < detections.Count; i++)
            {
                ...
                // Get the object class name (face model: classId always 0)
                var classname = (m_labels != null && detection.classId < m_labels.Length)
                    ? m_labels[detection.classId].Replace(" ", "_")
                    : "face";
                ...
                {
                    Debug.Log($"[FaceDetection] Raycast failed for detection {i} - no surface hit");
                    continue;
                }
